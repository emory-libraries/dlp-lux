...
RSpec.describe "Visibility requests", :clean, type: :request do
  ...
  describe "GET /catalog/:id" do
    context "as an unauthenticated user outside the Rose Reading Room" do
      ...
      it "does not load the 'show' page for a work with 'Irish Partner Sites' visibility" do
        get "/catalog/#{irish_partner_work_id}"
        expect(response.status).to eq 404
      end
    ...
    context "when from a Irish Partner Site" do
      it "loads the 'show' page for a work with 'Irish Partner Sites' visibility using Remote Address header" do
        get("/catalog/#{irish_partner_work_id}", headers: { "REMOTE_ADDR": irish_partner_ip })
        expect(request.headers["REMOTE_ADDR"]).to eq irish_partner_ip
        expect(response.status).to eq 200
      end

      it "loads the 'show' page for a work with 'Irish Partner Sites' visibility using X-Forwarded-For header" do
        get("/catalog/#{irish_partner_work_id}", headers: { "X-Forwarded-For": irish_partner_ip })
